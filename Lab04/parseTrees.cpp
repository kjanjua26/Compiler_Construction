/*
    This is the implementation of ParseTrees.
*/
#include <iostream>
#include <cstdlib>
#include <stack>
...
void postorder(BinaryTree *tree){
    // We print in post order to make sure that 
    if (tree != NULL){
...
void isTerminal(BinaryTree *curr){
    if(curr->getLeftChild() == NULL && curr->getRightChild() == NULL){
        cout << "Terminal: " << curr->getRootVal() << endl;
    }else{
        cout << "Non Terminal: " << curr->getRootVal() << endl;
        isTerminal(curr->getLeftChild());
        isTerminal(curr->getRightChild());
    }
}

int main() {
    BinaryTree *pt = buildParseTree("( ( 10 + 5 ) * ( 4 - 6 ) )");
    isTerminal(pt);
    postorder(pt);
    return 0;
}
